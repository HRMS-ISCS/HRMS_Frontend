  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({
        title: (
          <div className="flex items-center gap-2">
            <CheckCircle className={`h-5 w-5 ${isEmailSent ? "text-green-500" : "text-orange-500"}`} />
            <span>User Created Successfully</span>
          </div>
        ),
        description: (
          <div>
            ...
            {isEmailSent ? (
              <p className="text-xs mt-1 text-green-600 font-medium flex items-center gap-1">
                 ✉️ Welcome email sent to <span className="font-normal">{newUser.email}</span>
              </p>
            ) : (
