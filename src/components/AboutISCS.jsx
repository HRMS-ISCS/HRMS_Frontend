...
export default function AboutISCS() {
  ...
  return (
    <div
      className={`min-h-screen bg-white flex flex-col items-center justify-center relative overflow-hidden transition-opacity duration-500 ${isFadingOut ? "opacity-0" : "opacity-100"}`}
    >
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-r from-blue-100 to-green-100 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
      ...
      {/* Main Content */}
      <div
        className={`relative z-10 flex flex-col items-center justify-center space-y-8 px-8 transition-all duration-500 ${isFadingOut ? "scale-95" : "scale-100"} w-full`}
      >
        {/* ISCS Technologies Title with Gradient */}
        <div className="text-center mb-4">
          <h1 className="text-6xl font-bold">
        ...
        {/* Content with optional image */}
        <div
          className={`flex ${shouldShowImage ? "justify-between" : "justify-center"} items-center w-full max-w-6xl`}
        >
          {/* Display the current section with typing effect */}
          {currentSection < contentSections.length && (
            <div
              className={`${shouldShowImage ? "w-1/2 pr-8" : "w-full"} text-center ${contentClass}`}
            >
              {displayedText}
              <span className="inline-block w-1 h-6 bg-gray-600 ml-1 animate-pulse"></span>
            </div>
