...
export default function Employees() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showSortOptions && !event.target.closest('.relative')) {
  ...
  const fetchEmployees = useCallback(
    async (page = 1, sortField = null, order = "desc") => {
      setLoading(true);
      setError("");
      try {
        let url = `/db/employment-applications?page=${page}&page_size=${itemsPerPage}`;
       
        // Add sorting parameters if provided
        if (sortField) {
          url += `&sort_by=${sortField}&order=${order}`;
        }
  ...
  useEffect(() => {
    fetchEmployees(currentPage, sortBy, sortOrder);
  }, [currentPage, sortBy, sortOrder, fetchEmployees]);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          ...
          <div className="relative">
            ...
            >
              <Filter size={14} className="mr-1" />
              Sort By: {sortBy ? sortBy.replace('_', ' ') : 'Default'}
            ...
            {showSortOptions && (
              <div className={`absolute right-0 mt-1 w-48 rounded-md shadow-lg z-10 ${darkMode ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"}`}>
                <div className="py-1">
                  <button
                    className={`block w-full text-left px-4 py-2 text-sm ${sortBy === 'joining_date' ? (darkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-50 text-blue-700') : darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'}`}
                    onClick={() => {
                  ...
                  {/* <button
                    className={`block w-full text-left px-4 py-2 text-sm ${sortBy === 'employee_id' ? (darkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-50 text-blue-700') : darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'}`}
                    onClick={() => {
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 text-sm ${sortBy === 'full_name' ? (darkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-50 text-blue-700') : darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'}`}
                    onClick={() => {
                      setSortBy('full_name');
                      setShowSortOptions(false);
                    }}
