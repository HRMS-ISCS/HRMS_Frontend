...
export default function LoadingScreen({ onLoadingComplete }) {
  ...
  useEffect(() => {
    if (!loading && userData?.first_name) {
      let i = 0;
      const interval = setInterval(() => {
        setDisplayedName(userData.first_name.slice(0, i + 1));
        i++;
        if (i === userData.first_name.length) {
          clearInterval(interval);
          setNameAnimationComplete(true);
        }
      }, 100);
  ...
  useEffect(() => {
    if (nameAnimationComplete && userData) {
      const timer = setTimeout(() => handleTransition(), 1200);
      return () => clearTimeout(timer);
    }
  }, [nameAnimationComplete, userData]);
  ...
  return (
    <div
      className={`min-h-screen bg-white flex items-center justify-center relative overflow-hidden transition-opacity duration-500 ${
        isFadingOut ? "opacity-0" : "opacity-100"
      }`}
    >
      {/* BACKGROUND */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-r from-blue-100 to-green-100 rounded-full blur-3xl opacity-20 animate-blob" />
        <div className="absolute top-1/3 right-1/4 w-96 h-96 bg-gradient-to-r from-purple-100 to-pink-100 rounded-full blur-3xl opacity-20 animate-blob animation-delay-2000" />
        <div className="absolute bottom-1/4 left-1/3 w-96 h-96 bg-gradient-to-r from-yellow-100 to-red-100 rounded-full blur-3xl opacity-20 animate-blob animation-delay-4000" />
      </div>

      {/* CONTENT */}
      <div
        className={`relative z-10 flex flex-col items-center space-y-12 transition-all duration-500 ${
      ...
      >
        <div className="text-center space-y-6">
          ...
          {!loading && userData && nameAnimationComplete && (
            <div className="text-2xl font-medium text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-blue-500 to-green-500 animate-float-in">
              {userData.first_name} {userData.last_name}
            </div>
          )}
        ...
        <div className="w-96 h-3 bg-gray-200 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-purple-600 via-blue-500 to-green-500 animate-progress" />
        </div>

        <div className="flex gap-4">
          <div className="w-5 h-5 bg-purple-500 rounded-full animate-bounce" />
          <div className="w-5 h-5 bg-blue-500 rounded-full animate-bounce animation-delay-200" />
          <div className="w-5 h-5 bg-green-500 rounded-full animate-bounce animation-delay-400" />
        </div>
      </div>
    </div>
  );
